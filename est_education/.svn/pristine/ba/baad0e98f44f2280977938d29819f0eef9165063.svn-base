...
<body class="hold-transition sidebar-mini layout-fixed">
							...
							<div class="card card-primary">
								...
								<div class="card-body">
									...
									<div class="form-group row">
										...
										<div class="col-sm-4">
											<input type="text" class="form-control" id="inputRole" name="prjctRole"  value="${ projectOne.prjctRole }"  >
										</div>
									</div>
									<div class="form-group row">
										<label for="inputStrtDt" class="col-sm-2 col-form-label">프로젝트 시작일</label>
										<div class="col-sm-4">
											<input type="date" class="form-control" id="inputStrtDt" name="prjctStrtDt" value="${ projectOne.prjctStrtDt }"  >
										</div>
									...
									<div class="form-group row">
										...
										<div class="col-sm-4">
											<input type="text" class="form-control" id="inputMktgPt" name="prjctMktgPt" value="${ projectOne.prjctMktgPt }"  >
										</div>
									</div>
									<div class="form-group row">
									    <label for="inputWrkpl" class="col-sm-2 col-form-label">프로젝트 회사</label>
										<div class="col-sm-4">
											<input type="text" class="form-control" id="inputWrkpl" name="prjctWrkpl" value="${ projectOne.prjctWrkpl }"  >
										</div>		
									</div>		
								</div>
								<!-- /.card-body -->
	...
	function fn_selectskill(data, skillone, number, SkillArray) {
		var skillList = data.skillList;
 		var updateButton = '<td><button type="button" onclick="fn_skillupdatebutton('+number+')" class="btn btn-primary float-middle" data-toggle="modal" data-target="#modal-1">추가</button></td>'
		var cancleButton = '<td><button type="button" onclick="fn_cancleButton('+number+')" class="btn btn-primary float-middle">취소</button></td>'
		...
		body.append("<tr>" + "<td>" + cancleButton + "</td>" + "</tr>");
		$('#projectskillclose').click();
	}
	
 	function fn_cancleButton() {
		var comAjax = new ComAjax();
		comAjax.addParam("prjctSq", prjctSq);
		comAjax.setUrl("<c:url value='/projectskill/selectProjectSkillList.do' />");
		comAjax.setCallback(setting);
	...
	function selectAll(selectAll)  {
 	...
 	$(document).ready(function() {
			...
			$("#inputTyp").val("${ projectOne.prjctTyp }").prop("selected", true);
			...
			$("#updateButton").on("click", function() {
				...
				var comAjax = new ComAjax();
				comAjax.setUrl("<c:url value='updateProject.do' />"); //프로그램 URI
