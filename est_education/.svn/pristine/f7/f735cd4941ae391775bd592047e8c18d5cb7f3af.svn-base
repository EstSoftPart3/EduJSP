...
<%-- 회원 전화번호 3등분 --%>
<c:if test="${ fn:length(visitantVo.visiNum) > 0 }">
	<c:set var="Num1" value="${ fn:substring(visitantVo.visiNum, 0, 3) }" />
	<c:set var="Num2" value="${ fn:substring(visitantVo.visiNum, 4, 8) }" />
	<c:set var="Num3" value="${ fn:substring(visitantVo.visiNum, 9, 13) }" />
</c:if>
<style>
.text-right {
	text-align: right;
			...
			<div class="content-wrapper">
				...
				<div class="content-header">
					<div class="container-fluid">
						<div class="row mb-2">
							<div class="col-sm-6">
								<h1 class="m-0 text-dark">방문자 수정</h1>
							</div>
							<!-- /.col -->
							<div class="col-sm-6">
								<div class="btn-wrapper float-right">
									<a href='<c:url value="/visitant/openVisitantListPage.do" />' class="btn btn-info">목록으로</a>
									...
									<div class="form-group row">
										...
										<div class="col-sm-4">
											<input type="text" class="form-control" id="inputNm"
												name="visiNm" placeholder="이름" value="${ visitantVo.visiNm }">
										</div>
									</div>
									<div class="form-group row">
										...
										<div class="col-sm-4">
											<input type="date" class="form-control" name="visiDt"
												id="inputDt" value="${ visitantVo.visiDt }">
										</div>
									</div>
									<div class="form-group row">
									...
									<div class="form-group row">
										...
										<div class="col-sm-2">
											<select id="inputNum1" class="form-control">
												<option value="010" <c:if test="${ Num1 eq '010' }">selected</c:if> >010</option>
												<option value="011" <c:if test="${ Num1 eq '011' }">selected</c:if> >011</option>
												<option value="012" <c:if test="${ Num1 eq '012' }">selected</c:if> >012</option>
												<option value="013" <c:if test="${ Num1 eq '013' }">selected</c:if> >013</option>
												<option value="0507" <c:if test="${ Num1 eq '0507' }">selected</c:if> >0507</option>
											</select>
										</div>
										<div class="col-sm-4">
											<input type="text" class="form-control" id="inputNum2" maxlength="4" value="${ Num2 }" >
										</div>
										<div class="col-sm-4">
											<input type="text" class="form-control" id="inputNum3" maxlength="4" value="${ Num3 }" >
										</div>
									...
									<div class="form-group row">
										...
										<div class="col-sm-4">
											<input type="text" class="form-control" name="visiTem"
												id="inputTem" placeholder="ex)36.5" value="${ visitantVo.visiTem }">
										</div>
									</div>
									<div class="form-group row">
...
<%-- 												id="inputCd" placeholder="ex)면접, 직원, 기타" value="${ visitantVo.visiTcd }"> --%>
	...
	<script>
		var visiSq = "<c:out value='${visitantVo.visiSq}'/>";
		...
		$(document).ready(function() {
			...
			function fn_updateVisitantCallBack(data) {
				var code = data.code;
				if(code > 0) {	// 정상 추가시 방문자 목록 화면으로 이동
					alert("방문자 수정이 정상적으로 완료되었습니다.");
					var comSubmit = new ComSubmit();
					comSubmit.setUrl("<c:url value='/visitant/openVisitantDetailPage.do' />");
					comSubmit.addParam('sq', visiSq);
					comSubmit.submit();
				}else {
					alert("방문자 수정에 실패하였습니다.");
				}
			}
			...
			$("#updateBtn").on("click", function() {
				...
				comAjax.setUrl("<c:url value='/visitant/updateVisitant.do' />"); //프로그램 URI
				
				/* addParam 필수값만 셋팅 후 나머지는 값 있을시 추가 */
// 				console.log("udateAjax : " + visiSq);
				comAjax.addParam("visiSq", visiSq);
				...
				comAjax.setCallback(fn_updateVisitantCallBack);//콜백 선언
				comAjax.ajax();
			});
		});
	</script>
