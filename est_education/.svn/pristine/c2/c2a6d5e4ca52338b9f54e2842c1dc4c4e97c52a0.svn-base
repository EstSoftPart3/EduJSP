						...
						<div class="row mb-2">
							...
							<div class="col-sm-6">
								<div class="btn-wrapper float-right">
									<a href='<c:url value="openProjectListPage.do" />' class="btn btn-info">목록으로</a>
							...
							<div class="card card-primary">
								...
								<div class="card-body">
									...
									<div class="form-group row">
										...
										<label class="col-sm-4 col-form-label">${ projectOne.prjctTyp }</label>
									</div>
									<div class="form-group row">
										<label class="col-sm-2 col-form-label">목적</label>
										<label class="col-sm-4 col-form-label">${ projectOne.prjctRole }</label>
									</div>
									<div class="form-group row">
										<label class="col-sm-2 col-form-label">시작일</label>
										<label class="col-sm-4 col-form-label">${ projectOne.prjctStrtDt }</label>
									...
									<div class="form-group row">
										...
										<label class="col-sm-4 col-form-label">${ projectOne.prjctMktgPt }</label>
									</div>
									<div class="form-group row">
										<label class="col-sm-2 col-form-label">분산지점</label>
										<label class="col-sm-10 col-form-label">${ projectOne.prjctWrkpl }</label>
									</div>
								</div>
								<!-- /.card-body -->
							</div>
	...
	function fn_ProJectSkillCallBack(data) {
			...
			projectList.forEach(project => {
				...
				body.append(
						"<tr class='list-row'>" + 
							...
							"<td>" + element_isnull(project.prjctTyp) + "</td>" +
							"<td>" + element_isnull(project.prjctStrtDt) + "</td>" +
							"<td>" + element_isnull(project.prjctEndDt) + "</td>" +
							"<td>" + element_isnull(project.prjctAddr) + "</td>" +
							"<td><a href='<c:url value='openProjectUpdatePage.do?update=" + project.prjctSq + "' />' class='btn btn-primary float-center'>수정</a></td>" +
		...
		$(document).ready(function() {
			...
			$("#updateBtn").on("click", function() {
				var comSubmit = new ComSubmit();
				comSubmit.setUrl("<c:url value='openProjectUpdatePage.do?update=${ projectOne.prjctSq }' />");
			...
			$("#deleteBtn").on("click", function() {
				if(confirm("정말 삭제하시겠습니까?")) {
					var comAjax = new ComAjax();
					comAjax.setUrl("<c:url value='deleteProject.do' />");
